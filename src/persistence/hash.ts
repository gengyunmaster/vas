...
export async function hashBlob(blob: Blob): Promise<string> {
  ...
  // crypto.subtle requires a secure context; plain HTTP (e.g. testing over a
  // LAN IP) falls back to the software implementation so content addressing
...
export function sha256Hex(bytes: Uint8Array): string {
  ...
  for (let block = 0; block < paddedLength; block += 64) {
    ...
    for (let t = 0; t < 64; t++) {
      ...
      const ch = (e & f) ^ (~e & g);
      const temp1 = (h + bigS1 + ch + K[t] + w[t]) >>> 0;
  ...
  for (const [index, word] of hash.entries()) outView.setUint32(index * 4, word);
  return toHex(out);
}
